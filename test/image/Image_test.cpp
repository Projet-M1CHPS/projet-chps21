...
TEST(ImageTest, SaveAndLoadRandomImage) {
  std::filesystem::create_directory(
          "SaveAndLoadRandomImage_TestDir");   // create a dir to store the test images

  image::GrayscaleImage a = image::ImageSerializer::createRandomNoiseImage(128, 128);
  image::ImageSerializer::save("SaveAndLoadRandomImage_TestDir/testImage.png", a);
  image::GrayscaleImage b =
          image::ImageSerializer::load("SaveAndLoadRandomImage_TestDir/testImage.png");
...
TEST(ImageTest, LoadPngDirectory) {
  ASSERT_ANY_THROW(image::ImageSerializer::loadDirectory(
          "Image_test"));   // error, the file isnt a directory
  ASSERT_ANY_THROW(image::ImageSerializer::loadDirectory(
          "LoadPngDirectory_TestDir"));   // error, the directory doesnt exist
  std::filesystem::create_directory(
          "LoadPngDirectory_TestDir");   // create a dir to store the test images

  ASSERT_ANY_THROW(image::ImageSerializer::loadDirectory(
          "LoadPngDirectory_TestDir"));   // warning, the directory is empty
  ...
  std::vector<image::GrayscaleImage> img_list =
          image::ImageSerializer::loadDirectory("LoadPngDirectory_TestDir");
...
TEST(ImageTest, ImageGetters) {
  ...
  for (unsigned i = 0; i < a.getSize(); i++) { ASSERT_EQ(pixel_data_ptr[i], a.getPixel(i)); }
