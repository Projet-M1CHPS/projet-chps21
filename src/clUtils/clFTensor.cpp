...
namespace math {
  ...
  clFTensor::clFTensor(size_t x, size_t y, size_t z, utils::clWrapper &cl)
      : x_dim(x), y_dim(y), z_dim(z) {
    size_t size = x * y * z;
    data = cl::Buffer(cl.getContext(), CL_MEM_READ_WRITE, size * sizeof(float));
  }
  ...
  std::vector<clFMatrix> clFTensor::getMatrices() {
    ...
    for (size_t i = 0; i < z_dim; i++) {
      matrices.push_back(getMatrix(i));
    }
    return matrices;
  }
